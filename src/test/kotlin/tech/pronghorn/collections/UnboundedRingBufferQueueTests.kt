...
class UnboundedRingBufferQueueTests : PronghornTest() {
    ...
    fun powerOfTwoRequirementTest() {
        ...
        val queue = UnboundedRingBufferQueue<Int>(requestedCapacity)
    ...
    @RepeatedTest(lightRepeatCount)
    fun addTest() {
        val queue = UnboundedRingBufferQueue<Int>(smallQueueSize)
    ...
    fun unboundedTest() {
        ...
        assertEquals(smallQueueSize, queue.capacity)
        queue.add(random.nextInt())
        val newCapacity = smallQueueSize * 2
        assertEquals(newCapacity, queue.capacity)

        // force a wrap around the ring buffer
        while(queue.isNotEmpty()){
            queue.poll()
        }

        // ensure expanding still works
        while(!queue.isFull){
            queue.add(random.nextInt())
        }
        queue.add(random.nextInt())
    ...
    fun addAllUnboundedTest() {
        ...
        assertEquals(smallQueueSize, queue.capacity)
        assertTrue(queue.addAll(randomNumbers))
        assertEquals(smallQueueSize * 2, queue.capacity)
    }
